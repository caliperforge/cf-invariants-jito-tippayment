...
pub mod jito_tip_payment {
    ...
    pub fn change_tip_receiver(ctx: Context<ChangeTipReceiver>) -> Result<()> {
        ...
        // PLANTED BUG (cf-invariants-jito-tippayment, change_tip_receiver_state_update):
        //   Upstream commits the rotation by writing the new tip receiver into
        //   Config.tip_receiver here:
        //
        //       ctx.accounts.config.tip_receiver =
        //           ctx.accounts.new_tip_receiver.key();
        //
        //   This planted variant drops that write. The lamport-side effects of
        //   handle_payments (drain + credit) still run normally — so Solana's
        //   runtime balance check is not tripped — but Config.tip_receiver
        //   stays at its prior value. From the caller's perspective the
        //   instruction succeeded, yet the on-chain rotation never happened.
        //
        //   This surfaces as a violation of the
        //   `invariant_change_tip_receiver_updates_config` fixture, which
        //   reads Config.tip_receiver after each successful call and asserts
        //   it equals the new_tip_receiver argument the caller passed in.
