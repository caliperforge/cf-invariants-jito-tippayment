...
// cf-invariants-jito-tippayment port (anchor 0.31.1 -> 1.0.1):
//   Upstream tip-payment imports `use solana_program::loader_v4;` and uses it
//   exactly once (inside `is_program`, to check whether an account's owner is
//   the loader-v4 program). In the solana-3.x modular split that anchor-lang
//   1.0.1 brings in, the loader-v4 program ID lives in
//   `solana_sdk_ids::loader_v4`, alongside the other loader / sysvar / config
//   ID modules upstream already used from `solana_sdk_ids`. We import all of
//   them from solana-sdk-ids 3.x as a single source of truth and drop the
//   upstream `use solana_program::loader_v4;` line.
use solana_sdk_ids::{
    bpf_loader, bpf_loader_deprecated, bpf_loader_upgradeable, config, loader_v4, native_loader,
...
pub mod jito_tip_payment {
    ...
    pub fn change_block_builder(
        ...
        // PLANTED BUG (cf-invariants-jito-tippayment, change_block_builder_state_update):
        //   Upstream commits the rotation by writing the new block builder into
        //   Config.block_builder here:
        //
        //       ctx.accounts.config.block_builder =
        //           ctx.accounts.new_block_builder.key();
        //
        //   This planted variant drops that write. The commission_pct write
        //   below still runs, and the lamport-side effects of handle_payments
        //   (drain + credit) still run normally — so Solana's runtime balance
        //   check is not tripped — but Config.block_builder stays at its
        //   prior value. From the caller's perspective the instruction
        //   succeeded, yet the on-chain block-builder rotation never
        //   happened.
        //
        //   This surfaces as a violation of the
        //   `invariant_change_block_builder_updates_config` fixture, which
        //   reads Config.block_builder after each successful call and
        //   asserts it equals the new_block_builder argument the caller
        //   passed in.
...
impl TipPaymentAccount {
    ...
    fn initialize<'info>(
        ...
        // anchor-lang 1.0.1 port shim: `CpiContext::new` now takes the program
        // ID as a `Pubkey` (was `AccountInfo` in 0.31.1). Pass `*system_program.key`
        // to satisfy the new signature without changing CPI semantics.
